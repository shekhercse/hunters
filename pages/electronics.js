...
export async function getServerSideProps (context) {
  ...
  let electronics = {}
    for (let item of products) {
      if (item.title in electronics) {
        if (!electronics[item.title].color.includes(item.color) && item.availableQty > 0) {
          electronics[item.title].color.push(item.color)
        }
        if (!electronics[item.title].size.includes(item.size) && item.availableQty > 0) {
  ...
  return {
      props: { products: JSON.parse(JSON.stringify(electronics)) },// will be passed to the page component as props
