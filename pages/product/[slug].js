...
const Post = ({ buyNow, addToCart, product, variants }) => {
  ...
  // Below all are functions starting form const some are built in and some of custom defined functions
      ...
      <div className="container px-5 py-24 mx-auto">
        <div className="lg:w-4/5 mx-auto flex flex-wrap">
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            <h2 className="text-sm title-font text-gray-500 tracking-widest">Hunterstore</h2>
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              <div className="flex">
                ...
                {Object.keys(variants).includes('pink') && Object.keys(variants['pink']).includes(size) && <button onClick={() => { refreshVariant(size, 'pink') }} className={`border-2 ${color === 'pink' ? 'border-black' : 'border-gray-300'} ml-1 bg-pink-500 rounded-full w-6 h-6 focus:outline-none`}></button>}
                {Object.keys(variants).includes('red') && Object.keys(variants['red']).includes(size) && <button onClick={() => { refreshVariant(size, 'red') }} className={`border-2 ${color === 'red' ? 'border-black' : 'border-gray-300'} ml-1 bg-red-500 rounded-full w-6 h-6 focus:outline-none`}></button>}
                {Object.keys(variants).includes('yellow') && Object.keys(variants['yellow']).includes(size) && <button onClick={() => { refreshVariant(size, 'yellow') }} className={`border-2 ${color === 'yellow' ? 'border-black' : 'border-gray-300'} ml-1 bg-yellow-500 rounded-full w-6 h-6 focus:outline-none`}></button>}
              ...
              <div className="flex ml-6 items-center">
                ...
                <div className="relative">
                  <select value={size} onChange={(e) => { refreshVariant(e.target.value, color) }} className="rounded border appearance-none py-2 focus:outline-none focus:ring-2 focus:ring-pink-200 focus:border-pink-500 text-base pl-3 pr-10">
            ...
            <div className="flex">
              ...
              <button onClick={() => { buyNow(slug, 1, 499, product.title, product.size, product.color) }} className="flex ml-auto text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded">Buy Now</button>
              <button onClick={() => { addToCart(slug, 1, 499, product.title, product.size, product.color) }} className="flex ml-auto text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded">Add to Cart</button>
            ...
            <div className="pin mt-6 flex space-x-2 text-sm">
              ...
              <button onClick={checkServiceability} className="text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded">Check

              </button>
              {(!service && service != null) && <div className='text-red-700 text-sm mt-3'>
                Soory Service not available to this pincode
              </div>}
              {(service && service != null) && <div className='text-red-700 text-sm mt-3'>
                Yeh! Service is available to this pincode
              </div>}
            </div>
          </div>
        </div>
