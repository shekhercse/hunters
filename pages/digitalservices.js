...
const digitalservices = ({ products }) => {
...
export async function getServerSideProps (context) {
  ...
  let products = await Product.find({ category: 'digitalservices' })
  let digitalservices = {}
    for (let item of products) {
      if (item.title in digitalservices) {
        if (!digitalservices[item.title].color.includes(item.color) && item.availableQty > 0) {
          digitalservices[item.title].color.push(item.color)
        }
        if (!digitalservices[item.title].size.includes(item.size) && item.availableQty > 0) {
          digitalservices[item.title].size.push(item.size)
        }
      }
      else {
        digitalservices[item.title] = JSON.parse(JSON.stringify(item))
        if (item.availableQty > 0) {
          digitalservices[item.title].color = [item.color]
          digitalservices[item.title].size = [item.size]
        }
      }
    }
  return {
      props: { products: JSON.parse(JSON.stringify(digitalservices)) },// will be passed to the page component as props
