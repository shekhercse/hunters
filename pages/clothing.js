...
export async function getServerSideProps (context) {
  ...
  let clothing = {}
    for (let item of products) {
      if (item.title in clothing) {
        if (!clothing[item.title].color.includes(item.color) && item.availableQty > 0) {
          clothing[item.title].color.push(item.color)
        }
        if (!clothing[item.title].size.includes(item.size) && item.availableQty > 0) {
          clothing[item.title].size.push(item.size)
        }
      }
      else {
        clothing[item.title] = JSON.parse(JSON.stringify(item))
        if (item.availableQty > 0) {
          clothing[item.title].color = [item.color]
          clothing[item.title].size = [item.size]
        }
      }
    }
  return {
      props: { products: JSON.parse(JSON.stringify(clothing)) },// will be passed to the page component as props
